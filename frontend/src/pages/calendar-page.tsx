...
import { PanelRightClose, PanelRightOpen, Check } from "lucide-react";
...
function CalendarContent() {
    ...
    useEffect(() => {
        fetch(`${BASE_URL}/integrations/calendar`)
            .then(res => res.json())
            .then(data => {
                if (Array.isArray(data)) {
                    const parsedEvents: CalendarEvent[] = data.map((e, i) => ({
                        id: e.id || `evt-${i}`,
                        title: e.title || "Untitled Event",
                        description: `Attendees: ${e.attendees ? e.attendees.join(", ") : ""}`,
    ...
    const handleEventAdd = async (event: CalendarEvent) => {
        ...
        try {
            const res = await fetch(`${BASE_URL}/integrations/calendar`, {
                method: 'POST',
            ...
            <div className="border-t pt-4">
                ...
                <div className="flex flex-col gap-1">
                    {etiquettes.map((item) => (
                        <button
                            key={item.id}
                            onClick={() => toggleColorVisibility(item.color)}
    ...
    return (
        <CalendarProvider>
            <div className="flex h-full flex-1 overflow-hidden">
                ...
                {panelOpen && (
                    <div className="w-[280px] shrink-0 border-l border-border bg-background text-foreground overflow-y-auto scrollbar-none">
                        <RightPanel />
                    </div>
                )}
                <Button
                    variant="ghost"
                    size="icon"
                    className="absolute right-4 top-4 z-10 size-8 text-muted-foreground hover:text-foreground"
