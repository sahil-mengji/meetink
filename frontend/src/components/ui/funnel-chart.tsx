...
export function PatternLines({
  ...
  let path = `M 0 ${height} L ${width} 0 M ${width/2} ${height*1.5} L ${width*1.5} ${height/2} M -${width/2} ${height/2} L ${width/2} -${height/2}`;
  if (orientation.includes("vertical")) path = `M ${width / 2} 0 L ${width / 2} ${height}`;
  if (orientation.includes("horizontal")) path = `M 0 ${height / 2} L ${width} ${height / 2}`;

  return (
    <pattern id={id} width={width} height={height} patternUnits="userSpaceOnUse">
      {background && <rect width={width} height={height} fill={background} />}
      <path d={path} stroke={stroke} strokeWidth={strokeWidth} strokeLinecap="square" />
    </pattern>
  );
}
...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-semibold text-foreground text-sm truncate max-w-[120px] sm:max-w-[160px]">
      {display}
    </span>
  );
  const pctEl = showPercentage && (
    <span className="rounded-full bg-foreground px-3 py-1 font-bold text-background text-xs shadow-sm shrink-0">
      {formatPercentage(pct)}
    </span>
  );
  const labelEl = showLabels && (
    <span className="max-w-[120px] sm:max-w-[160px] md:max-w-[200px] truncate md:whitespace-normal md:wrap-break-word text-center font-medium text-muted-foreground text-xs line-clamp-2">
      {stage.label}
    </span>
  );
